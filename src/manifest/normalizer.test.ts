...
import { normalizeRepository, normalizeManifest } from './normalizer.js';
import type { Repository, Manifest } from './types.js';
...
describe('normalizer', () => {
  ...
  describe('normalizeRepository', () => {
    it('should remove invalid categories and set needs_review', () => {
      ...
      const { repo: normalized, changes } = normalizeRepository(repo, mockTaxonomy);

      expect(normalized.categories).toEqual(['unclassified']);
      expect(normalized.needs_review).toBe(true);
      expect(changes).toContain('Removed invalid categories: infrastructure, cli-tools');
      expect(changes).toContain('All categories invalid, defaulting to unclassified');
    });
    ...
    it('should canonicalize categories (case/whitespace)', () => {
      ...
      if (changes.length > 0) {
        expect(changes.some(c => c.includes('Canonicalized') || c.includes('categories'))).toBe(true);
      }
    });
    ...
    it('should invalidate invalid framework and set needs_review', () => {
      ...
      expect(normalized.needs_review).toBe(true);
      expect(changes).toContain('Invalid framework "invalid-framework" -> null');
    });
  ...
  describe('normalizeManifest', () => {
    it('should normalize all repositories and provide summary', () => {
      ...
      expect(result.manifest.repositories[0].categories).toEqual(['unclassified']);
    ...
    it('should update manifest_updated_at timestamp', () => {
      ...
      const result = normalizeManifest(manifest);
      const after = new Date(result.manifest.manifest_metadata.manifest_updated_at);

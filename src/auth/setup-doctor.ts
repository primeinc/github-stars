...
function readEnv(): Parameters<typeof resolveAuthMode>[0] {
	const requested = (process.env.AUTH_MODE_REQUEST || "auto").trim() as
		| AuthMode
		| "auto";
	...
	const fb = (process.env.PAT_FALLBACK_TO_GITHUB_TOKEN || "true")
		.trim()
		.toLowerCase();
	...
	const appFetch = (process.env.GITHUB_APP_SUPPORTS_FETCH || "true")
		.trim()
		.toLowerCase();
	return {
	...
	try {
		...
		if (err instanceof AuthConfigError) {
			...
			process.stderr.write(
				`Missing config: ${err.missing_config.join(", ")}\n`,
			);
...
if (process.argv[1]?.endsWith("setup-doctor.ts")) {
	main();
}

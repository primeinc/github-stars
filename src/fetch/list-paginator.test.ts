...
const QUERY =
	"query($cursor: String) { viewer { starredRepositories(after: $cursor) { __typename } } }";
		...
		const oct = fakeOctokit([
			{
				viewer: {
					starredRepositories: {
						edges: [
							{
								node: { nameWithOwner: "a/b", isPrivate: false },
								starredAt: "2025-01-01T00:00:00Z",
							},
							{
								node: { nameWithOwner: "c/d", isPrivate: true },
			...
			},
			{
				viewer: {
					starredRepositories: {
						edges: [
							{
								node: { nameWithOwner: "e/f", isPrivate: false },
	...
	it("extracts partial data when graphql throws an org-blocked error", async () => {
		const orgBlockedErr = Object.assign(new Error("Request failed"), {
			data: {
				viewer: {
					starredRepositories: {
						edges: [
							{
								node: { nameWithOwner: "x/y", isPrivate: false },
			...
			errors: [
				{
					message:
						"`acme-corp` forbids access via a personal access token (classic).",
				},
			],
		});
		...
		expect(r.list).toEqual([
			{ repo: "x/y", user_starred_at: "2025-01-04T00:00:00Z" },
		]);

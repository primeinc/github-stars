...
| §2 Build, Test, Validation | Single bullet, "JSON Schema validation in CI workflows 02/04" | Table separating **JSON Schema** (`cardinalby` `mode: default`, in 02 + 03) from **Taxonomy** (`pnpm validate`, in 00-ci + 03) | The two validators check different invariants. The old wording also miscounted workflows (04 doesn't validate). Caught by Copilot review on PR #58. The mode value originally landed as `strict` (which the action rejects) and was corrected to `default` in `02M-mode-correction.md`. |
...
| §6 03-classify gates description | "normalizes + strict-validates against taxonomy" | "pnpm normalize → cardinalby (mode: default) HARD GATE → pnpm validate (taxonomy strict) HARD GATE" | Reflects the gate reorder in `03-classify-repos.yml` for 02F (`mode: lax` → `mode: default`). The originally-shipped `mode: strict` was invalid and was corrected in `02M-mode-correction.md`. |

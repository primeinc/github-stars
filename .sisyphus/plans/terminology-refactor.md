...
> **Status: ARCHIVED — Completed and Superseded.** The renames listed below
> have been applied: `repos.yml` and `schemas/repos-schema.json` use
> `user_starred_at`, `repo_pushed_at`, `manifest_updated_at`, and
> `classified_at`. Frontend references in this doc to `docs/app.js` and
> `docs/index.html` are stale: those are Vite build outputs of `web/src/`.
> Do not edit files under `docs/`; they are regenerated by
> `04-build-site.yml`. Kept for historical context only.

...
> **Status: ARCHIVED — Superseded.** This plan targets `docs/app.js` and
> `docs/index.html`, which are now Vite **build outputs** (see
> `web/vite.config.js`, `outDir: ../docs`). The site source has moved to
> `web/src/App.jsx`. Sort/filter behavior is implemented there. Do not
> hand-edit files under `docs/`; they are regenerated by

...
function inheritEntry(fresh, prior) {
  ...
  const out = {
    ...
    last_synced_sha: prior.last_synced_sha || fresh.last_commit_sha || '0'.repeat(40),
...
function main() {
  ...
  console.log('Loading classification snapshot (web/public/data.json @ 2026-01-29)...');
  ...
  // Build lookup maps
  const currentByName = new Map(currentRepos.filter(r => r?.repo).map(r => [r.repo, r]));
  const snapshotByName = new Map(snapshotRepos.filter(r => r?.repo).map(r => [r.repo, r]));
  ...
  console.log(`  ${fromCurrent} repos kept classification from current repos.yml (197 prior)`);
  console.log(`  ${fromSnapshot} repos inherited classification from web/public/data.json snapshot`);
